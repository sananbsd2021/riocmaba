...
const PaginatedBookRePage = () => {
  ...
  return (
    <div className="container p-4">
      <h1 className="text-2xl font-bold mb-4">ทะเบียนคำสั่ง</h1>
        ...
        <table className="min-w-full table-auto border-collapse border border-gray-200 shadow-md">
          <thead className="bg-gray-100">
            <tr>
              ...
              <th className="px-4 py-2 border border-gray-200 text-left">เลขทะเบียนรับ</th>
          ...
          <tbody>
            {posts.map((post, index) => (
              <tr key={post.numreceive} className="hover:bg-gray-50">
                <td className="px-4 py-2 border border-gray-200">{index + 1 + (currentPage - 1) * pageSize}</td>
                <td className="px-4 py-2 border border-gray-200">{post.numreceive}</td>
                <td className="px-4 py-2 border border-gray-200">{post.date}</td>
                <td className="px-4 py-2 border border-gray-200">{post.fromreceive}</td>
                <td className="px-4 py-2 border border-gray-200">{post.toreceive}</td>
                ...
                <td>
                  <Link href={`/bookre/${post.id}`} className="px-4 py-2 bg-gray-600 text-white rounded">View</Link>
                </td>
              </tr>
            ))}
          </tbody>
