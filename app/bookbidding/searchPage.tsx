  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { data, error } = await supabase
        .from("bookbidding") // เปลี่ยนชื่อเป็นตารางที่คุณต้องการค้นหา
        .select("*")
          ...
          <ul className="space-y-2">
            {results.map((result: any) => (
              <li key={result.numbid} className="p-2 border rounded">
                <p>
                  <strong className="text-xl">ที่คำสั่ง:</strong>{" "}
                  {result.numbid}
                </p>
                <p>
                  <strong className="text-xl">เรื่อง:</strong> {result.topic}
                </p>
                <p>
                  <strong className="text-xl">วันที่:</strong> {result.date}
                </p>
                <p>
                  <strong className="text-xl">ปี:</strong> {result.year}
                </p>
              </li>
            ))}
          </ul>
...
export default SearchBookbidForm;
